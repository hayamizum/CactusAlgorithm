...
def readcsv(filename):
    ...
    with open(filename, 'r', newline='', encoding='utf-8-sig') as file:
        ...
        for i, row in enumerate(reader):
            if i == 0:
                # 1行目は行列の大きさの情報
                # The first line is about the number of objects n
                n = int(row[0])
                continue
            # 2行目以降のデータを数値としてリストに追加します
            # from the second line, the number is appended behind the list,
...
def mkAdj(V):
    ...
    for x, y in itertools.permutations(V, 2):  # 枚举所有的情况
        Adj[x, y] = 1
        for z in V - {x, y}:  # 如果存在z在x到y的最短路上，就直接切断x到y的直接连接
    ...
    for x in V:
        if CountNeighbor(x, Adj) == 0:#如果是孤立点，直接删除
            isDeleted = 1
            V_del.add(x)
        elif CountNeighbor(x, Adj) == 1:#如果是叶子节点，直接删除，同时还要记录邻接关系
            isDeleted = 1
            for i in range(len(Adj)):
                if Adj[x, i] == 1:
                    u = i
            Adj_opt[x, u] = Adj_opt[u, x] = 1 #记录在临时的邻接关系上面
            V_del.add(x)
        elif CountNeighbor(x, Adj) == 2:#如果是不分歧的点，就直接删除
            isDeleted = 1
            Neighbor = set()
            for i in range(len(Adj)):
                if Adj[x, i] == 1:
...
edge_labels = nx.get_edge_attributes(G,0)#是否输出权重
...
sequence_labels = [
    'B.CN.1994.94CNKM001.AB213667.SH', 'B.CN.1995.95CNKM002.AB213668.SH',
    'B.CN.1996.96CNKM004.AB213670.SH', '08_BC.CN.1996.96CNKM005.AB213671.SH',
    'B.CN.1996.96CNKM006.AB213672.SH', '07_BC.CN.1997.97CNKM007.AB213673.SH',
    '07_BC.CN.1997.97CN001_C54.AF286226.PI', '08_BC.CN.1998.98CN006.AF286229.NR',
    '07_BC.CN.1998.98CN009.AF286230.NR', '08_BC.CN.1997.97CNGX_7F.AY008716.PI',
    'C.CN.1998.YNRL9840.AY967806.PI', 'B.CN.1999.plwj.GU177863.PB',
    'B.CN.1999.plwj11_6.GU647196.PB', 'B.CN.1999.plwj5_6.GU647197.PB',
    'B.CN.1999.plwj2_6.GU647198.PB', 'B.CN.1998.YN9802.JF932495.PI',
    'B.CN.1998.YN9838.JF932496.PI'
]

labels = {node: label for node, label in zip(G.nodes(), sequence_labels)}
#
node_sizes = [100 if 0 <= node <= 19 else 20 for node in G.nodes()]
edge_colors= ['white' if 0<=node<=19 else 'black' for node in G.nodes()]

nx.draw_networkx(G, pos=pos, with_labels=False, node_color=node_color, edgecolors=edge_colors, node_size=node_sizes)
...
# df.to_csv(r'C:\Users\26798\Documents\graph_random.csv', index=False)
